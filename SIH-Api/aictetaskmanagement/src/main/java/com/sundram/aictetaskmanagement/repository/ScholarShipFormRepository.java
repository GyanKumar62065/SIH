...
public interface ScholarShipFormRepository extends JpaRepository<ScholarshipForm,Integer>{
   ...
   @Query(value= "select * from scholarship_forms where application_id =  ?1", nativeQuery = true)
   ...
   @Query(value = "select * from scholarship_forms where is_scholared = 0 and is_rejected = 0", nativeQuery = true)
